...
function setPictureToPagePDF(progSlide:PictureObj, doc:jsPDF) {  
  if (progSlide.fillColor === "none") {
    ...
    doc.rect(+progSlide.position.x, +progSlide.position.y, +progSlide.wigth, +progSlide.height, "F")
  }
  if (progSlide.borderColor !== "unset"){
    const borderColor = getColorForJsPDF(progSlide.borderColor)
    doc.setDrawColor(borderColor[0], borderColor[1], borderColor[2])
    doc.rect(+progSlide.position.x, +progSlide.position.y, +progSlide.wigth, +progSlide.height, "S")
  }
  return doc
}
...
function setShapeToPagePDF(progSlide:ShapeObj, doc:jsPDF) {
  ...
  if (drawType !== "") {
    if (progSlide.type ===  "circle")
      doc.ellipse(+(progSlide.position.x + progSlide.wigth / 2), +(progSlide.position.y + progSlide.height / 2), 
        +progSlide.wigth / 2, +progSlide.height / 2, drawType)
      ...
      doc.triangle(+progSlide.position.x + +progSlide.wigth/2, +progSlide.position.y, 
        ...
        +progSlide.position.x + +progSlide.wigth, +progSlide.position.y + +progSlide.height, drawType)
    else
      doc.rect(+progSlide.position.x, +progSlide.position.y, +progSlide.wigth, +progSlide.height, drawType)
  }
  return doc
}
...
function setTextBackgrondToPagePDF(progSlide:TextObj, doc:jsPDF){
  ...
  if (drawType !== "")
    doc.rect(+progSlide.position.x - borderLag, +progSlide.position.y - borderLag, +progSlide.wigth + borderLag * 2, +progSlide.height + borderLag * 2, drawType)
  return doc
}
...
function setTextLinesToPagePDF(progSlide:TextObj, doc:jsPDF) {
  ...
  while (startPosition <= progSlide.text.length) {
    if (progSlide.text[startPosition] === '\n' || startPosition === progSlide.text.length) {
      ...
      if (CanEl.width > progSlide.wigth)
        CanEl.width = +progSlide.wigth-parseInt(progSlide.fontSize) * 0.05
      if (ctx != null) {
